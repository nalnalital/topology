// File: mesh.js - Structure maillage avec cases et transformations
// Desc: En français, dans l'architecture, je suis la couche d'abstraction maillage avec structure case=[[x0,y0],[x1,y1],[x2,y2],[x3,y3]] et fonction createSurface pour transformations linéaires
// Version 1.0.0 (structure initiale selon vision utilisateur)
// Author: DNAvatar.org - Arnaud Maignan
// Date: June 08, 2025 21:00 UTC+1
// Logs:
//   - v1.0.0: Structure maillage avec cases coordonnées grille + createSurface()
 ...
 */
export function createMesh(width = 30, height = 20) {
  const all = [];
 ...
 */
export function transformMesh(maillage, params = {}) {
 ...
 */
export function debugCase(case_quad, index = -1) {
  const prefix = index >= 0 ? `Case[${index}]` : 'Case';
  console.log(`❌ [debugCase][mesh.js] ${prefix}: ${JSON.stringify(case_quad)}`);
}
 ...
 */
export function debugMesh(maillage) {
  console.log(`❌ [debugMesh][mesh.js] Maillage ${maillage.width}x${maillage.height} = ${maillage.totalCases} cases`);
  
  if (maillage.all.length > 0) {
    console.log(`❌ [debugMesh][mesh.js] Première case: ${JSON.stringify(maillage.all[0])}`);
    console.log(`❌ [debugMesh][mesh.js] Dernière case: ${JSON.stringify(maillage.all[maillage.all.length - 1])}`);
  }
  
  if (maillage.transformed) {
    console.log(`❌ [debugMesh][mesh.js] Transformé avec params: ${JSON.stringify(maillage.params)}`);
  }
} 